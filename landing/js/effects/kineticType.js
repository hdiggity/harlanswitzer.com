...
export function kineticType({ letters, stage }, pointer){
  ...
  letters.forEach((el, i) => {
    ...
    // edge letters blur/fade when cursor is far to one side
    const posD = (i - mid) / Math.max(1, mid);
    const edgeFade = Math.abs(posD) * Math.abs(gnx) * (1 - prox);
    const bl = edgeFade * 1.8;
    const op = clamp(1 - edgeFade * 0.32, 0.55, 1);
